...
output$eval.table <- renderText({
  kable(data.frame(test = eval()), format = 'html', escape = F) %>%
    kable_styling(
      full_width = F,
...
renderFormattable({
  formattable(vi() %>%
                ...
                select(predictorNameUnit, mean, everything(), -X) %>%
                mutate_if(is.numeric, round) %>%
                arrange(desc(mean)),
              align = c("l","c"), 
              list(mean = color_bar("orange"), 
                   brt.KDE = formatter("span", style = x ~ style(color = ifelse(rank(-x) <= 4, "green", "gray")), 
                                       x ~ sprintf("%.2f", x, rank(-x))),
                   brt.target = formatter("span", style = x ~ style(color = ifelse(rank(-x) <= 4, "green", "gray")), 
                                       x ~ sprintf("%.2f", x, rank(-x)))                
                   )
  )
})
