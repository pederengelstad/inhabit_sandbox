...
img_uri <- function(x) { sprintf('<center><img src="%s" width="550"></center>', knitr::image_uri(x)) }
...
pickerInput(inputId = 'sp_select', label = 'Species of interest:'
            ...
            , width = '95%'
            , choicesOpt = list(subtext = paste0("(", sp_list$latin, ')', sep='')), selected = "Wavyleaf basketgrass"
...
vi = reactive({
  ...
  read.csv(paste0("species/",gsub(' ','',tolower(input$sp_select)),'/vi_table.csv',sep='')
      , header = T, stringsAsFactors = F,  encoding = "ISO-8859-1", fileEncoding = "ISO-8859-1")
})
# server component
output$vi_table <- renderText({
  vi() %>%
    ...
    mutate_if(is.numeric, function(x) {
      cell_spec(x, color = "white", background = spec_color(1:10, end = 0.9, option = "B", direction = -1))
    }) %>%
    kable(caption = vi.caption, format = 'html', escape = F) %>%
      kable_styling(font_size = 12, full_width = F, bootstrap_options = c("condensed", "responsive"))
})
```
...
output$source_table <- renderText({
  kable(pred.info %>% 
          inner_join(vi(), by = c('nameunits'='name')) %>%
